...
const _supplyTypes = ['paper', 'ink', 'toner', 'other'];

Future<void> showBulkAddServiceSuppliesDialog(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...rows.asMap().entries.map((entry) {
                    ...
                    final isPaper = row['supplyType'] == 'paper';
                    return Padding(
                      padding: const EdgeInsets.only(bottom: 8),
                      child: Row(
                        children: [
                            ...
                            child: Container(
                              ...
                              child: DropdownButtonHideUnderline(
                                child: DropdownButton<String>(
                                  value: row['supplyType'] as String,
                                  ...
                                  onChanged: (v) => setDialogState(() {
                                    row['supplyType'] = v ?? 'paper';
                                    if (v != 'paper') row['paperSize'] = '';
                                  }),
                                ),
                              ),
                            ),
                          ...
                          Expanded(
                            flex: 2,
                            child: bulkTextField(
                              hint: isPaper ? 'e.g. short, a4' : '—',
                              initialValue: row['paperSize'] as String,
                              onChanged: isPaper ? (v) => row['paperSize'] = v : (_) {},
                            ),
                          ),
                          const SizedBox(width: 8),
                          Expanded(
                  ...
                  : () async {
                      ...
                      if (validRows.isEmpty) {
                        ScaffoldMessenger.of(ctx).showSnackBar(
                          const SnackBar(
                              content: Text('Enter at least one supply name')),
                        );
                        return;
                      }
                      ...
                      try {
                        ...
                        for (final row in validRows) {
                          final type = row['supplyType'] as String;
                          final paperSize = type == 'paper' &&
                                  (row['paperSize'] as String).isNotEmpty
                              ? (row['paperSize'] as String).trim()
                              : null;
                          await inventoryProviderRef.createServiceSupply(
                            ServiceSupply(
                              id: 0,
                        ...
                        activityLogRef.log(
                          ...
                          description: 'Bulk added $saved service supplies',
                        );
                        if (ctx.mounted) Navigator.pop(ctx);
                        onRefresh();
                      } catch (e) {
                        debugPrint('Bulk add supplies error: $e');
                        if (ctx.mounted) {
